...
import SavingsAcLadgerPrint from './savingsLadgerPrint';

export default function SavingsLadger(props) {
    ...
    let fine = 0
    const linkToLadger = () => {
        navigate.push('/Home/SavMainHome/interestbreakdown', { data: props.location.state.data });
    };
    ...
    return (
        <Container maxWidth="false" style={{ padding: 0 }}>
            <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                ...
                <h2>Savings Ladger</h2>
                <div>
                      <Button variant='contained' color='primary'  style={{ marginRight:10 }} onClick={() => linkToLadger()}>
                        Interest Breakdown
                      </Button>
                    <Button
                        variant='contained'
            ...
            >
                <PDFExport keepTogether='.keep' paperSize="LEGAL" margin={{ top: 10, right: 10, bottom: 10, left: 10 }} fileName='sav_ladger.pdf' ref={pdfExportComponent}>
                    <SavingsAcLadgerPrint data={recData} info={{ ac_no: Data.ac_no, name: Data.name }} />
                </PDFExport>
            </div>
