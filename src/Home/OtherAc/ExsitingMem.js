...
const handleChange = (event)=>{
    ...
    const linkToLoan = (props,key)=>{
        updateSelected(key)
        history.push('/Home/OtherHome/OtherHolDet/', {"data" : props})
    }
    const linkToLadger = (props,key)=>{
        updateSelected(key)
        history.push('/Home/OtherHome/OtherLadger/', {"data" : props, change : Math.random()})
    }

    const [isrefresh, setIsRefres] = React.useState(false)
    React.useEffect(()=>{
    
        sendAsync('allSav').then((result)=>{
   ...
   const ListGenerate = ()=>
    allLoan.map((value,key) =>{
        ...
        return (
            ...
            <ListItem button key={key} autoFocus={true} onClick={()=>linkToLoan(value,key)} selected={selected === key}>
                <ListItemAvatar>
                <Avatar>
                <Example data={data} />
                </Avatar>
                ...
                <OtherTrans callback={value} onClick={()=>updateSelected(key)} />
