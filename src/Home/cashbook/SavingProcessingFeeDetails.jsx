...
export default function SavingProcessingFeeDetails() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'SavingProcessingFeeDetails', {
        method: 'POST',
        body: JSON.stringify({
          from: from,
          to: to,
      ...
      <Paper style={{ marginBottom: '20px' }}>
        <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Typography variant='h5' style={{ textAlign: 'center', margin: '10px', marginTop: '20px', fontWeight: 'bold' }}>
            Savings Account Processing Fee Details
          </Typography>
          <div></div>
        </Toolbar>
        <TableContainer >
            ...
            <TableBody>
              {data.slice(startIndex, endIndex).map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell >Rs.{Number(item.proc_chrg).toFixed(2)}</TableCell>
