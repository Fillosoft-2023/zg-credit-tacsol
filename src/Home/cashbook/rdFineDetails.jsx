...
export default function RdFineDetails() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'rdTransDetails', {
        method: 'POST',
        body: JSON.stringify({
          from: from,
          to: to,
    ...
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      <Paper style={{ marginTop: '20px', marginBottom: '20px' }}>
        <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Typography variant='h5' style={{ textAlign: 'center', margin: '10px', marginTop: '20px' }}>
            Recurring Deposit Account Fine Details
          </Typography>
          <div></div>
        </Toolbar>
        <TableContainer >
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{ fontWeight: 'bold' }}>Fine Amount</TableCell>
