...
export default function RdProcessingFeesDetails() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'rdProcDetails', {
        method: 'POST',
  ...
  return (
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      ...
      <IconButton onClick={() => history.goBack()} style={{ marginLeft: '10px' }}>
        ...
        <Typography variant='h5' style={{textAlign:'center',margin:'10px',marginTop:'20px'}}>To RD Processing Fees Account Details</Typography>
        <hr/>
        <TableContainer >
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{fontWeight:'bold'}}>Processing Charges</TableCell>
            ...
            <TableBody>
              {data.slice(startIndex, endIndex).map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell >{item.proc_chrg}</TableCell>
