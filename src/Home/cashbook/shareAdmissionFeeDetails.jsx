...
export default function ShareAdmissionFeeDetails() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'ShareAdmissionFeeDetails', {
        method: 'POST',
        body: JSON.stringify({
          from: from,
          to: to,
      ...
      if (result.princ_details && Array.isArray(result.princ_details)) {
        console.log(result.princ_details);
      } else {
    ...
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      ...
      <Paper style={{ marginTop: '20px', marginBottom: '20px' }}>
        <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Typography variant='h5' style={{ textAlign: 'center', margin: '10px', marginTop: '20px' }}>By Share member Application fees A/C</Typography>
          <div></div>
        </Toolbar>
        <TableContainer >
          <Table>
            ...
            <TableBody>
              {data.slice(startIndex, endIndex).map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell >{item.app_fees}</TableCell>
