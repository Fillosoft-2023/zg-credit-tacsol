...
export default function ShareDetailsA() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'shareDetailsA', {
        method: 'POST',
  ...
  return (
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      ...
      <IconButton onClick={() => history.goBack()} style={{ marginLeft: '10px' }}>
        ...
        <Typography variant='h5' style={{textAlign:'center',margin:'10px',marginTop:'20px'}}>To Share A/C(A) Details</Typography>
        <div></div>
        </Toolbar>
        <hr/>
        <TableContainer >
