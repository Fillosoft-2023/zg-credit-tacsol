...
export default function ShareDetailsB() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'shareDetailsB', {
        method: 'POST',
        body: JSON.stringify({
          from: from,
          to: to,
    ...
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      ...
      <Paper style={{ marginTop: '20px', marginBottom: '20px' }}>
        <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Typography variant='h5' style={{ textAlign: 'center', margin: '10px', marginTop: '20px' }}>To Share A/C(B) Details</Typography>
          <div></div>
        </Toolbar>
        <TableContainer >
          <Table>
