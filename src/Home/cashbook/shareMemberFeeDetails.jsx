...
export default function ShareMemberFeeDetails() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'ShareMemberFeeDetails', {
        method: 'POST',
        body: JSON.stringify({
          from: from,
          to: to,
    ...
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      ...
      <Paper style={{ marginTop: '20px', marginBottom: '20px' }}>
        <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Typography variant='h5' style={{ textAlign: 'center', margin: '10px', marginTop: '20px' }}>By Share member fees A/C</Typography>
          <div></div>
        </Toolbar>
        <TableContainer >
          <Table>
            ...
            <TableBody>
              {data.slice(startIndex, endIndex).map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell >{item.shr_amt}</TableCell>
