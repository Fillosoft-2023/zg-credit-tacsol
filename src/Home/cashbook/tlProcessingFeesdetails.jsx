...
export default function TlProcessingFeeDetails() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(Api + 'taDetails', {
        method: 'POST',
  ...
  return (
    <Container maxWidth="xxl" style={{ minHeight: '75vh' }}>
      ...
      <Toolbar component={Paper} style={{ display: 'flex', }}>
                    ...
                    <Typography variant='h5' style={{ textAlign: 'center' }}>TL Processing Fees Details</Typography>
                </Toolbar>
        <TableContainer >
          <Table>
            <TableHead>
            ...
            <TableBody>
              {data.slice(startIndex, endIndex).map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell >{item.prc_chr}</TableCell>
