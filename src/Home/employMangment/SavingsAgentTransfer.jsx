...
function SavingsAgentTransfer() {
    ...
    const fetchSavAccounts = () => {
        setLoading(true);
        fetch(Api + 'getSavAccountsForTransfer', {
            method: 'POST',
            ...
            .then(data => {
                if (data.success) {
                    setSavAccounts(data.savAccounts || []);
            ...
            .catch(err => {
                ...
                showSnackbar('Error fetching savings accounts', 'error');
                setLoading(false);
            });
    };
    ...
    const handleTransfer = () => {
        if (selectedAccounts.length === 0) {
            showSnackbar('Please select at least one savings account', 'warning');
            return;
        }
    ...
    const handleSelectAll = (event) => {
        if (event.target.checked) {
            setSelectedAccounts(savAccounts.map(account => account.id));
        } else {
            setSelectedAccounts([]);
        }
    };
            ...
            <TableContainer component={Paper} style={{height:'60vh',overflow:'scroll'}}>
                <Table>
                    <TableHead>
                        <TableRow>
                            <TableCell padding="checkbox">
                                <Checkbox
                                    indeterminate={selectedAccounts.length > 0 && selectedAccounts.length < savAccounts.length}
                                    checked={savAccounts.length > 0 && selectedAccounts.length === savAccounts.length}
                                    onChange={handleSelectAll}
                                />
                            </TableCell>
                            <TableCell>Reg No.</TableCell>
                            <TableCell>Savings No.</TableCell>
                    ...
                    <TableBody>
                        ...
                        ) : savAccounts.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={9} align="center">
                                    No savings accounts found for this agent
                                </TableCell>
                            </TableRow>
                        ) : (
                            savAccounts.map(account => {
                                const currentBalance = calculateCurrentBalance(account);
                                return (
                                    <TableRow key={account.sav_no} hover>
                                        <TableCell padding="checkbox">
                                            <Checkbox
                                                checked={selectedAccounts.includes(account.sav_no)}
                                                onChange={() => handleSelectAccount(account.sav_no)}
                                            />
                                        </TableCell>
                                        <TableCell>{account.ac_no}</TableCell>
                                        <TableCell>{account.sav_no}</TableCell>
                                        <TableCell style={{ fontWeight: 'bold' }}>
                                            {account.account_holder_name || 'N/A'}
                                        </TableCell>
                                        
                                        <TableCell>
                                            ₹{Number(account.tot_dep || 0).toLocaleString('en-IN')}
                                        </TableCell>
                                        <TableCell>
...
export default SavingsAgentTransfer;    