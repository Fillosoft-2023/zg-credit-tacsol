...
import AllFdPrint from './allFdPrint';

export default function AllFdAccountsInfo() {
        ...
        <Container component={Paper} maxWidth="false" style={{ padding: 0 }}>
            <Toolbar style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography variant='h6' style={{ fontWeight: 'bold' }}>All FD Accounts</Typography>
                </div>
                    ...
                    <TableBody>
                        {
                            data.map((item, index) =>
                                <TableRow key={index} style={{ backgroundColor: item.status === 'closed' ? '#7dfa9a' : '#fff' }}>
                                    ...
                                    <TableCell>{item.opn_dt}</TableCell>
                                    <TableCell>{item.tnre}</TableCell>
                                    <TableCell>{item.int_rt}%</TableCell>
                                    <TableCell>{Number(item.ta_amt).toFixed(2)}</TableCell>
                                    <TableCell>{Number(item.dep_bal).toFixed(2)}</TableCell>
                                    <TableCell>{item.mt_stus}</TableCell>
            ...
            <div style={{ marginLeft: '-100000px' }}>
                ...
                <PDFExport forcePageBreak='.keep' paperSize="LEGAL" margin={{ top: 5, right: 10, bottom: 5, left: 10 }} fileName='all_fd_accounts.pdf' landscape ref={pdfExportComponent}>
                    <AllFdPrint data={sliceData} />
                </PDFExport>
            </div>
