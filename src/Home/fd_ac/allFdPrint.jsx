...
export default function AllFdPrint(props){
    ...
    data.map((item,index)=>{
        ...
        <div className={index > 0 ? 'keep' : ''} key={index}>
            ...
            <Typography style={{margin: 0,textAlign: 'right',fontSize : 10}}>Page {index+1}/{data.length}</Typography>
                ...
                <Typography style={{fontSize : 12}}>All Fixed Deposit Accounts</Typography>
            </div>
                ...
                </tr>
                {
                    item.map((child_item,child_index)=>
                        <tr key={child_index}>
                            ...
                            <td className="td">{child_item.rd_no}</td>
                            <td className="td">{child_item.c_nmbr}</td>
                            <td className="td" style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 150 }}>{child_item.vill}</td>
                            <td className="td">{DateHandler(child_item.opn_dt)}</td>
                            <td className="td">{child_item.tnre}</td>
                            <td className="td">{child_item.int_rt}</td>
                            <td className="td">{child_item.ta_amt}</td>
                            <td className="td">{child_item.dep_bal}</td>
                            <td className="td">{child_item.mt_stus}</td>
