...
export default function CreateFd(props) {
    ...
    const handleMths = () => {
        ...
        let int = Number(data.fd_amount) * Number(intererst) / 100;
        ...
        if (!isNaN(matAmt)) {
            ...
            Object.assign(data, { int_amt: totInt.toFixed(2), mt_amt: matAmt.toFixed(2) });
    ...
    return (
        <Container className={classes.container} >
            <Toolbar style={{ justifyContent: 'space-between' }}>
                <IconButton onClick={() => navigate.push('/Home/FdHome')}>
                    <ArrowBack />
                </IconButton>
                <Typography variant='h6'>Enter TL Details</Typography>
                <Button
                    size='small'
                    variant="contained"
                    color=''
