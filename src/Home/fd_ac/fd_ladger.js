...
import FdMaturityCertificate from './fdMaturityCertificate';
...
export default function FdLadgerBook(props) {
    ...
    const maturityExportComponent = React.useRef(null);
        ...
        <Container maxWidth="lg" style={{ width: '100%', padding: 0 }}>
            <Toolbar component={Paper} style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div>
                    ...
                    <Tooltip title="Generate Maturity Certificate">
                    <Button
                        style={{ marginLeft: 10 }}
                        variant="contained"
                        color="primary"
                        disabled={Data.mt_stus === "not donejyoti"}
                        size="small"
                        onClick={() => {
                            if (maturityExportComponent.current) {
                                maturityExportComponent.current.save();
                            }
                        }}
                    >
                ...
                <Table size="small">
                    <TableHead>
                        <TableRow style={{backgroundColor:'#888888'}}>
                            ...
                            <TableCell style={{fontWeight:'bold',color:'white'}}>BALANCE</TableCell>
                            ...
                            data.map((item, index) => {
                                ...
                                return (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell style={{ textTransform: 'capitalize' }}>{item.type === 'dep' ? 'deposit' : item.type === 'with' ? 'withdrwal' : 'intrest'}</TableCell>
                                        <TableCell>Rs.{(dep + int_dep) - withd}</TableCell>
            ...
            >
                ...
                <PDFExport keepTogether='.keep' paperSize="LEGAL" margin={{ top: 10, right: 10, bottom: 10, left: 10 }} fileName='fd_maturity_certificate.pdf' ref={maturityExportComponent}>
                    <FdMaturityCertificate data={data} info={{
                        ac_no: Data.ac_no,
                        name: Data.name,
                        f_name: Data.f_name,
                        vill: Data.vill,
                        per_pin: Data.per_pin,
                        c_nmbr: Data.c_nmbr,
                        int_rt: Data.int_rt,
                        mt_dt: Data.mt_dt,
                        mt_amt: Data.mt_amt,
                        ...
                        relation_with_nom: Data.relation_with_nom
                    }} />
                </PDFExport>
            </div>
