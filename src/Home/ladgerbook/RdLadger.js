...
import MyDocument from './../../pdf/juornal/rdMaturity'
import GetAppIcon from '@material-ui/icons/GetApp';
import PrintIcon from '@material-ui/icons/Print';
export default function RdMatured(){
    ...
    return (
        ...
        <Typography style={{margin: 5,marginRight: 20,textAlign: 'center'}}>RD Maturity records</Typography>
            <TextField
                type="date"
                variant="outlined"
                size="small"
            ...
            {
                ready ? (   
                    <PDFDownloadLink document={<MyDocument callback={send} DateDiffer={date} />} fileName={filename+"RDMaturityRecord.pdf"}>
                        ...
                        <TableBody>
                            {
                                Data.map((item,index)=> {
                                    ...
                                    sum += Number(item.m_amnt)
                                    return (
                                        <TableRow key={index}>
                                            <TableCell>{item.opn_dte}</TableCell>
                                            <TableCell>RD matured from {item.ac_no}</TableCell>
                                            <TableCell style={{color: 'red'}}>{item.m_amnt}</TableCell>
