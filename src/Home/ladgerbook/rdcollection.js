...
import MyDocument from './../../pdf/juornal/rdColRecord'
import GetAppIcon from '@material-ui/icons/GetApp';
import PrintIcon from '@material-ui/icons/Print';
export default function Rdcollect(){
    ...
    return (
        ...
        <Paper className={classes.dateBar}>
            <div>
            <Typography style={{margin: 5,marginRight: 20,textAlign: 'center'}}>Rd Collection records</Typography>
            ...
            {
                ready ? (   
                    <PDFDownloadLink document={<MyDocument callback={send} DateDiffer={date} />} fileName={filename+"RDcollectionRecord.pdf"}>
                        ...
                        <TableBody>
                            {
                                Data.map((item,index)=> {
                                    ...
                                    sum += Number(item.recv_amnt)
                                    return (
                                        <TableRow key={index}>
                                            <TableCell>{item.recived_date}</TableCell>
                                            <TableCell>RD from {item.ac_no}</TableCell>
                                            <TableCell style={{color: '#42f548'}}>{item.recv_amnt}</TableCell>
