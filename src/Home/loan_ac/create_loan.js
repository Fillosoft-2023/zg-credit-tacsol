...
import emi from '../../consts/loan_emi_structure';
...
export default function CreateLoan(props){
    ...
    const handleDialog = ()=>{
        ...
        if(valid()){
            handleMths()
            setDialogOpen(true)
            setData({...data,intrestrate : data.tenure === "12" ? 36 : 54,noofemi :  data.tenure === "12" ? 50 : 70});
        }
    ...
    const handleSubmit = ()=> {
        ...
        if(valid()){
            ...
            fetch(Api+'loan_create',{
                method : 'POST',
                body : JSON.stringify({...data,remarks : 'structured'})
            })
            .then(res=>res.json())
            .then(res=>{
                console.log(res)
   ...
   // code for handle automatic math
    const handleMths = ()=> {
        ...
        let total_interest = data.loanamount * (data.intrestrate / 100);
        let total_recoverable = Number(data.loanamount) + Number(total_interest);
        let emi_amount = total_recoverable / data.noofemi;
        let emi_princ = data.loanamount / data.noofemi;
        let emi_int = total_interest / data.noofemi;
        ...
        // let princ_at_one_emi = Number(data.loanamount) / Number(freq)
        ...
        if(!isNaN(emi_int)){
            ...
            Object.assign(data, {emiamt : emi_amount, totint : total_interest, totamount : total_recoverable, int_per_emi : emi_int, princ_per_emi : emi_princ });
    ...
    return ( 
        ...
        <Typography>Enter Loan Details</Typography>
        ...
        <Divider style={{marginBottom: 10,marginTop: 10}} />
            <Grid container spacing={2}  >
                ...
                <Grid item xs={12} sm={6}>
                    <FormControl variant="outlined"  fullWidth>
                        <InputLabel id="demo-simple-select-outlined-label">Loan Tenure</InputLabel>
                        <Select
                            labelId="demo-simple-select-outlined-label"
                            id="demo-simple-select-outlined"
                            error={err.tenure}
                        ...
                        {
                            Object.keys(emi).map((item,index)=><MenuItem key={index} value={item}>{item}</MenuItem>)
                        }
                ...
                <Grid item xs={12} sm={6}>
                    <FormControl variant="outlined"  fullWidth>
                        <InputLabel id="demo-simple-select-outlined-label">Loan Amount</InputLabel>
                        <Select
                            labelId="demo-simple-select-outlined-label"
                            id="demo-simple-select-outlined"
                            error={err.loanamount}
                        ...
                        {
                            Object.keys(emi[data.tenure]).map((item,index)=><MenuItem key={index} value={item}>{item}</MenuItem>)
                        }
