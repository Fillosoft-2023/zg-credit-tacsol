...
export default function FdTransaction(props) {
  ...
  const handleClickOpen = () => {
    ...
    // })
    fetch(Api+'collectInfoPlociy&reffer_id='+props.callback.reffer_id)
    .then(res=>res.json())
    ...
    .catch(err=>{
       setMassg({
         open : true,
         severity : 'error',
         massg : 'Faild to connect to the se'
       })
    })
          ...
          <div className={style.balence}>
              
                {
                  DataCollect.map((item)=>{
                    ...
                    return (
                      <Typography color="red" >Account Balance {Number(item.dep) + Number(item.ints) - Number(item.withs)}</Typography>
                    )
                  })
                }
