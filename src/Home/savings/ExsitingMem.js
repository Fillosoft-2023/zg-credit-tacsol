...
export default function ExistingHolder(props) {
    ...
    const linkToLoan = (props, key) => {
        updateSelected(key)
        history.push('/Home/SavHome/SavHolDet/', { "data": props })
    }
    const linkToLadger = (props, key) => {
        updateSelected(key)
        history.push('/Home/SavHome/SavLadger/', { "data": props, change: Math.random() })
    }
                ...
                <>
                    <ListItem button key={key} autoFocus={true} style={{ backgroundColor: value.status === 'closed' ? '#8bf7b2' : '#fff' }} onClick={() => linkToLoan(value, key)} selected={selected === key}>
                        ...
                        <ListItemText
                            ...
                            secondary={<Typography className={classess.fontSec} style={{ display: 'flex' }}>{value.acno}<Typography className={classess.fontSec} style={{ color: 'red' }}>/{value.rd_no}</Typography></Typography>}
                        />
                        <ListItemSecondaryAction style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>

                            <SavTrans callback={value} onClick={() => updateSelected(key)} />
