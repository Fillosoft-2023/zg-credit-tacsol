...
import { HdOutlined, OutlinedFlag } from '@material-ui/icons';
const menus = [
    {
        name: 'Daily',
        value: 1
...
export default function LoanSheet(prop) {
    ...
    const [filterDate, setFilterDate] = React.useState({ from: '', to: '' })
    ...
    React.useEffect(() => {
        ...
        if (filterDate.from && filterDate.to) {
            newData = data.filter((item) => {
                // Ensure robust date parsing. If item.opn_dte is "YYYY-MM-DD", new Date() works.
                // If it is invalid, we shouldn't filter it out blindly, or we might hide everything.
                // Assuming standard ISO format or YYYY-MM-DD from PHP.

                let from = new Date(filterDate.from).setHours(0, 0, 0, 0);
                let to = new Date(filterDate.to).setHours(23, 59, 59, 999);
                let itemDate = new Date(item.opn_dte).getTime();

                // If itemDate is invalid, decide whether to show or hide. 
                // Currently, we will hide it if it doesn't match the valid range.
                if (isNaN(itemDate)) {
                    // Check if it's DD-MM-YYYY format (common in some regions)
                    // Simple check: split and rearrange if needed, OR just return false
                    return false;
                }
        ...
        setNewData(sliceIntoChunks(newData, 16));
    ...
    const handleLoanByType = (prop) => {
        ...
        fetch(Api + 'loanSheet_by_type', {
            ...
            .then(res => {
                let data = res;
                if (propData.from && propData.to) {
                    data = res.filter((item) => {
                        let from = new Date(propData.from).getTime();
                        let to = new Date(propData.to).getTime();
                        let date = new Date(item.opn_dte).getTime();
